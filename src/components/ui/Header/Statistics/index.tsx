...
export default function Statistics() {
  ...
  const [globalStats, setGlobalStats] = useState<{ totalPlayers: number; avgAttempts: number | null } | null>(null);
  useEffect(() => {
    fetch(`/api/global-stats?mode=${mode}&date=${today}`)
      .then((res) => res.json())
  ...
  return (
    <StatisticsContainer>
      ...
      {activeTab === "community" && (
        <GlobalSection>
          <h2>{t("statistics.globalTitle")}</h2>
          {globalStats ? (
            <>
              <TodayRow>
                <span>{t("statistics.globalPlayers")}</span>
                <span>{globalStats.totalPlayers}</span>
              </TodayRow>
              <TodayRow>
                <span>{t("statistics.globalAvg")}</span>
                <span>{globalStats.avgAttempts ?? "—"}</span>
              </TodayRow>
            </>
          ) : (
            <TodayRow>
